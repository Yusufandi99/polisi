document.querySelectorAll(".nav-link").forEach((tab) => {
  ...
  async function loadData(tabId, searchQuery = "") {
    try {
      const response = await fetch(
        `/api/dispo?closing=${tabId === 1 ? 0 : 1}` // Menggunakan closing, bukan id_status
      );
  ...
  async function getDeskripsiDevisi(noDisposisi) {
    try {
      ...
      const response = await fetch(`/api/get-devisi?no_disposisi=${noDisposisi}`);
  ...
  async function updatePagination(tabId, data, page = 1) {
    ...
    for (const item of paginatedData) {
      ...
      getDeskripsiDevisi(item.no_disposisi).then((deskripsi) => {
        ...
        function getColorFromText(text) {
          ...
          return `btn-${colors[Math.abs(hash) % colors.length]}`;
        }
        ...
        detailButton.classList.add(getColorFromText(deskripsi));
      });
  
      if (item.closing !== 1) {
        // Hanya tampil jika closing !== 1
        ...
        editButton.addEventListener("click", function () {
          if (window.editIndexUrl) {
            localStorage.setItem("no_disposisi", item.no_disposisi);
            console.log(
              "No Disposisi tersimpan di localStorage:",
              item.no_disposisi
            );
        ...
        prosesButton.addEventListener("click", function () {
          if (window.prosesIndexUrl) {
            ...
            console.error("Route URL untuk proses.index tidak didefinisikan.");
          }
        });
      ...
      riwayatButton.addEventListener("click", function () {
        if (window.riwayatIndexUrl) {
          ...
          console.error("Route URL untuk riwayat.index tidak didefinisikan.");
        }
      });
    ...
    if (showingInfo) {
      showingInfo.textContent = `Showing ${startIndex + 1} to ${endIndex} of ${
        data.length
      } entries`;
    }
